          ...
          {items.map((p, i) => (
            <Reveal key={p.title} delay={i * 0.07}>
              <div className="project-card group bg-card border border-border rounded-md p-6 h-full flex flex-col overflow-hidden hover:border-neon/30 transition-all duration-300">
                ...
                <h3 className="relative z-10 font-display font-semibold text-foreground text-[16px] uppercase tracking-tight mb-2">{p.title}</h3>
                <p className="relative z-10 text-[13px] text-muted-foreground leading-relaxed mb-4 flex-1">{renderDesc(p.desc)}</p>
                ...
                {p.link ? (
                  ...
                  <span className="relative z-10 inline-flex items-center gap-2 font-sans text-[12px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50 border border-dim/50 px-4 py-2.5 mt-auto w-fit cursor-default">{t('projects.btn_soon')}</span>
                )}
              </div>
            </Reveal>
          ))}
